...
type Draft = { title: string; category: string; sourceUrl: string; commitmentText: string; baselineTimestamp: string; beneficiary: string; stake: string; expiresAt: string; contestWindow: string };
const initialDraft: Draft = { title: "", category: "", sourceUrl: "", commitmentText: "", baselineTimestamp: "", beneficiary: "", stake: "", expiresAt: "", contestWindow: "86400" };
...
export function validateDraft(draft: Draft, step: number, max = { title: 120, category: 48, url: 512, text: 2000 }) {
  ...
  if (step === 2 || step === 4) {
    if (!/^\d{14}$/.test(draft.baselineTimestamp)) errors.push("Baseline must be an exact 14-digit archive timestamp.");
  }
  if (step === 3 || step === 4) {
...
export function CreateFlow() {
  ...
  const write = getUpholdContractAddress() && commitmentId ? buildUpholdWrite("create_commitment", [commitmentId, draft.title.trim(), draft.category.trim(), draft.sourceUrl.trim(), draft.commitmentText.trim(), draft.beneficiary, draft.baselineTimestamp, new Date(draft.expiresAt).toISOString(), Number(draft.contestWindow)]) : null;
  ...
  return <><PageHeader eyebrow="Create commitment" title="Put money behind your word." copy="A published promise becomes enforceable only when its source, evidence baseline, and bond are explicit." />{!getUpholdContractAddress() && <SetupState compact />}<div className="create-layout"><div className="form-panel"><div className="stepper">{["Promise", "Evidence", "Bond", "Review"].map((label, index) => <div key={label} className={`step ${step === index + 1 ? "step-active" : ""} ${step > index + 1 ? "step-done" : ""}`}><span>{step > index + 1 ? <Check size={13} /> : index + 1}</span>{label}</div>)}</div>{errors.length > 0 && <div className="notice notice-error"><CircleHelp size={17} /><div>{errors.map((error) => <p key={error}>{error}</p>)}</div></div>}
      {step === 1 && <div className="form-section"><div className="section-intro"><h2>Define the promise</h2><p>The exact wording is the baseline validators compare against authenticated archived documents.</p></div>{input("Title", "title", { placeholder: "Monthly transparency report" })}{input("Category", "category", { placeholder: "Public accountability" })}<label className="field"><span>Published source URL</span><div className="input-with-icon"><ExternalLink size={15} /><input value={draft.sourceUrl} onChange={(event) => update("sourceUrl", event.target.value)} placeholder="https://example.com/commitment" type="url" /></div><small>Use the public page where this wording is published.</small></label><label className="field"><span>Exact commitment text</span><textarea value={draft.commitmentText} onChange={(event) => update("commitmentText", event.target.value)} placeholder="I will publish…" rows={5} /><small>{draft.commitmentText.length}/{max.text} characters</small></label></div>}
      {step === 2 && <div className="form-section"><div className="section-intro"><h2>Pin the baseline evidence</h2><p>Give the contract an exact Internet Archive capture to authenticate. Client-side checks are advisory; the contract remains authoritative.</p></div><label className="field"><span>Archive timestamp</span><input value={draft.baselineTimestamp} onChange={(event) => update("baselineTimestamp", event.target.value)} placeholder="20260101000000" inputMode="numeric" maxLength={14} /><small>Exactly 14 digits in YYYYMMDDhhmmss UTC.</small></label><div className="evidence-callout"><ShieldCheck size={18} /><div><strong>Evidence admission happens on-chain</strong><p>Uphold checks CDX metadata, pins the exact capture, enforces byte and content gates, hashes the body, then asks validators one bounded semantic question.</p></div></div></div>}
      ...
      {step === 4 && <div className="form-section"><div className="section-intro"><h2>Review before signing</h2><p>Review the exact call that will be sent to the Uphold contract. The wallet will confirm the bond value and Transaction Kit fee separately.</p></div><div className="review-grid"><div><span>Commitment ID</span><strong>{commitmentId || "Generated on review"}</strong></div><div><span>Promise</span><strong>{draft.title}</strong><p>{draft.commitmentText}</p></div><div><span>Source</span><strong>{draft.sourceUrl}</strong><p>Baseline {draft.baselineTimestamp}</p></div><div><span>Beneficiary</span><strong>{draft.beneficiary}</strong></div><div><span>Bond</span><strong>{draft.stake} GEN</strong><p>Contest protection: {Number(draft.contestWindow) / 3600} hours</p></div><div><span>Expiry</span><strong>{draft.expiresAt ? new Date(draft.expiresAt).toLocaleString() : "—"}</strong></div></div><div className="review-note"><LockKeyhole size={17} /><span>Stake is not withdrawable before settlement or clean completion.</span></div></div>}
