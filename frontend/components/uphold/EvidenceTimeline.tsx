...
export function EvidenceTimeline({ history }: { history: HistoryEntry[] }) {
  ...
  return <ol className="evidence-timeline">{history.toReversed().map((entry, index) => {
    const pendingAssessment = entry.captured_state === "AUTHENTICATED" && entry.snapshot_state === "UNASSESSED";
    return <li key={`${entry.event}-${entry.at}-${index}`} className="timeline-entry"><div className={`timeline-icon timeline-icon-${entry.event.toLowerCase()}`}><EventIcon entry={entry} /></div><div className="timeline-content"><div className="timeline-heading"><div><h3>{eventTitle(entry.event)}</h3><span>{formatDate(entry.at ?? "", true)}</span></div>{pendingAssessment ? <span className="classification classification-indeterminate">Authenticated · Pending adjudication</span> : entry.classification && <ClassificationBadge classification={entry.classification} />}</div>{(entry.capture_timestamp ?? entry.archive_timestamp) && <div className="evidence-meta"><span>{pendingAssessment ? "Authenticated · Pending semantic assessment · " : "Captured "}{formatDate(entry.capture_timestamp ?? entry.archive_timestamp ?? "", true)}</span>{(entry.sha256 ?? entry.archive_digest) && <code>{(entry.sha256 ?? entry.archive_digest ?? "").slice(0, 16)}…</code>}</div>}{entry.excerpt && <blockquote>“{entry.excerpt}”</blockquote>}{entry.short_reason && <p>{entry.short_reason}</p>}{entry.reason && <p className="timeline-reason"><span>{entry.domain ?? "Infrastructure"}</span> {entry.reason}</p>}{entry.event === "CHECK" && <p className={`qualification ${entry.qualified === false ? "qualification-muted" : ""}`}>{entry.qualified === false ? "Not counted toward the breach streak" : entry.classification === "HOLDS" ? "Resets the consecutive-negative streak" : entry.classification === "INDETERMINATE" ? "No economic effect · indeterminate" : "Qualified evidence point · advances the streak"}</p>}</div></li>;
  })}</ol>;
}
