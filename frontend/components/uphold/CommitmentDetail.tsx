...
export function CommitmentDetail({ commitmentId }: { commitmentId: string }) {
  ...
  const submitAction = (action: ActionKey) => {
    ...
    try {
      if (action === "increase_stake") {
        ...
        request = { tx: buildUpholdWrite("contest_breach", [commitmentId]), commitmentId, stateExpectation: { status: "CONTESTED" }, stateCheck: async () => (await readCommitment(commitmentId)).status === "CONTESTED" };
      } else {
  ...
  if (commitmentQuery.isError || !commitment) return <div className="notice notice-error"><CircleAlert size={17} /><span>Commitment {commitmentId} could not be read from Studio-dev.</span><Link href="/app/explore" className="inline-link">Back to explore</Link></div>;
