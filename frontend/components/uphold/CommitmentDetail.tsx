...
const lifecycle = ["Created", "Monitoring", "Breach claimed", "Settlement", "Completed / settled"];

function actionLabel(action: ActionKey): string { return ({ check: "Check now", increase_stake: "Increase stake", extend: "Extend expiry", contest: "Contest breach", adjudicate: "Adjudicate contest", settle: "Settle breach", expire: "Complete commitment" } as Record<ActionKey, string>)[action]; }
  ...
  const submitAction = (action: ActionKey) => {
    ...
    try {
      if (action === "increase_stake") {
        ...
        if (!/^\d{14}$/.test(contestTimestamp)) { setFormError("Use the exact 14-digit archived timestamp."); return; }
        request = { tx: buildUpholdWrite("contest_breach", [commitmentId, contestUrl.trim(), contestTimestamp]), commitmentId, stateExpectation: { status: "CONTESTED" }, stateCheck: async () => (await readCommitment(commitmentId)).status === "CONTESTED" };
      } else {
        ...
        const expected = action === "settle" ? "SETTLED" : action === "expire" ? "COMPLETED" : action === "adjudicate" ? undefined : commitment.status;
