...
const eventLabels: Record<string, string> = { CREATED: "created a commitment", CHECK: "checked archived evidence", CHECK_FAILURE: "recorded an evidence failure", STAKE_INCREASED: "increased the bond", EXTENDED: "extended expiry", BREACH_CLAIMED: "claimed breach", CONTEST_FILED: "filed a contest", CONTEST_UPHELD: "won a contest", CONTEST_REJECTED: "lost a contest", BREACH_CONFIRMED: "confirmed breach", SETTLED: "settled stake", COMPLETED: "completed intact" };
function EventIcon({ event }: { event: string }) { if (event === "CHECK" || event === "CREATED") return <Archive size={16} />; if (event.includes("BREACH")) return <CircleAlert size={16} />; if (event === "SETTLED" || event === "COMPLETED") return <LockKeyhole size={16} />; if (event.includes("CONTEST")) return <FileText size={16} />; return <RefreshCw size={16} />; }
