...
const domains: ErrorDomain[] = ["PRECONDITION", "EXTERNAL_EVIDENCE", "TRANSIENT", "MODEL_ERROR", "TRANSACTION_FAILURE", "EXECUTION_FAILURE", "STATE_CONFIRMATION_FAILURE"];
...
export function mapUpholdError(error: unknown, fallback: ErrorDomain = "TRANSACTION_FAILURE"): { domain: ErrorDomain; message: string } {
  ...
  if (/archive|capture|wayback|cdx|evidence/i.test(message)) return { domain: "EXTERNAL_EVIDENCE", message };
...
export function errorCopy(domain: ErrorDomain): string {
  return ({ PRECONDITION: "The current contract state does not allow this action.", EXTERNAL_EVIDENCE: "Evidence could not be authenticated. No breach judgment was made.", TRANSIENT: "This may be temporary. Reconcile the same transaction or try the read again.", MODEL_ERROR: "The bounded semantic result was invalid or unavailable. No breach evidence was counted.", TRANSACTION_FAILURE: "The wallet or fee flow did not submit this transaction.", EXECUTION_FAILURE: "The transaction finalized, but contract execution was not successful.", STATE_CONFIRMATION_FAILURE: "Execution completed, but the expected contract state was not confirmed yet." } as Record<ErrorDomain, string>)[domain];
}
