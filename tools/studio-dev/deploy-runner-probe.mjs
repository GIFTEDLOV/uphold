...
const chainId = 61997;
const probePath = path.resolve(root, "artifacts/runner-probe/studio-dev-runner-probe.py");
const outputDir = path.resolve(root, "artifacts/runner-probe");
const pendingPath = path.join(outputDir, "manifest.pending.json");
const completedPath = path.join(outputDir, "manifest.json");
const runner = "py-genlayer:1jb45aa8ynh2a9c9xn3b7qqh8sm5q93hwfp7jqmwsfhh8jpz09h6";
const profile = JSON.parse(readFileSync(path.resolve(root, "frontend/fee-profile.json"), "utf8"));
...
const source = readFileSync(probePath);
const sourceSha256 = sha256(source);

if (existsSync(pendingPath)) {
  const existing = JSON.parse(readFileSync(pendingPath, "utf8"));
  if (existing.txHash) throw new Error("Runner probe already broadcast; refusing a second probe deployment.");
}
if (existsSync(completedPath)) throw new Error("Runner probe already completed; refusing a second probe deployment.");
...
if (actualChain !== chainId || actualNetwork !== chainId) throw new Error(`Wrong probe chain: eth_chainId=${actualChain}, net_version=${actualNetwork}`);
...
if (feeValue <= 0n || balanceWei <= feeValue) throw new Error("Probe fee quote or balance is invalid.");

mkdirSync(outputDir, { recursive: true });
const manifest = {
  kind: "disposable-runner-probe",
  network: "GenLayer Studio-dev",
  rpc,
  chainId,
  jsChain: "studioDevnet",
  probePath: "artifacts/runner-probe/studio-dev-runner-probe.py",
  probeSha256: sourceSha256,
  runner,
  ...
  feeProfileSha256: sha256(readFileSync(path.resolve(root, "frontend/fee-profile.json"))),
  feeQuote: {
    source: "developer/measured profile plus current live prices",
    feeValue,
...
console.log(JSON.stringify({ PROBE_SHA256: sourceSha256, RUNNER: runner, RPC: rpc, CHAIN_ID: chainId, ESTIMATED_FEE_VALUE: feeValue.toString() }, null, 2));
...
save(pendingPath, manifest);
console.log(`PROBE_DEPLOYMENT_TX_HASH: ${txHash}`);
...
if (manifest.finalStatus !== "FINALIZED" || !isSuccessful(receipt)) {
  ...
  save(pendingPath, manifest);
  throw new Error(`Runner probe finalized unsuccessfully: ${manifest.finalStatus}/${manifest.executionResult}`);
}

const contractAddress = receipt.txDataDecoded?.contractAddress ?? receipt.data?.contract_address ?? receipt.contractAddress;
if (!contractAddress) throw new Error("Successful runner probe receipt did not include a contract address.");
manifest.contractAddress = contractAddress;
manifest.codePresent = Boolean(await client.getContractCode(contractAddress));
manifest.ping = await client.readContract({ address: contractAddress, functionName: "ping", args: [], transactionHashVariant: TransactionHashVariant.LATEST_FINAL });
if (manifest.ping !== "UPHOLD_RUNNER_PROBE_OK") throw new Error(`Unexpected runner probe readback: ${manifest.ping}`);
manifest.status = "FINALIZED_SUCCESSFUL";
save(pendingPath, manifest);
save(completedPath, manifest);
console.log(JSON.stringify({ PROBE_FINAL_STATUS: manifest.finalStatus, PROBE_EXECUTION_RESULT: manifest.executionResult, PROBE_ADDRESS: contractAddress, PROBE_READBACK: manifest.ping, CODE_PRESENT: manifest.codePresent }, null, 2));
